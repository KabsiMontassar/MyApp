...
export class TacheService {
  ...
  getTachesByEmployee(employeeId: number): Observable<Tache[]> {
    return this.http.get<Tache[]>(`${this.apiUrl}/employee/${employeeId}`);
  }
  ...
  ajouterSousTache(parentId: number, sousTache: Tache): Observable<Tache> {
    return this.http.post<Tache>(
      `${this.apiUrl}/add-sous-tache/${parentId}`,
      sousTache
    );
  }
  ...
  updateSousTache(id: number, sousTache: Tache): Observable<Tache> {
    return this.http.put<Tache>(
      `${this.apiUrl}/update-sous-tache/${id}`,
      sousTache
    );
  }
  ...
  getProgressionParEmployee(employeeId: number): Observable<any> {
    return this.http.get<any>(
      `${this.apiUrl}/progression-employe/${employeeId}`
    );
  }
  ...
  getNombreTachesParStatut(
    employeeId: number
  ): Observable<Record<string, number>> {
    return this.http.get<Record<string, number>>(
      `${this.apiUrl}/nombre-taches-par-statut/${employeeId}`
    );
  }

  getTachesParStatut(employeeId: number, statut: StatutTache): Observable<any> {
    return this.http.get<any>(
      `${this.apiUrl}/tache-par-statut/${employeeId}/${statut}`
    );
  }

  getHistoriqueParPeriode(
    employeeId: number,
    periode: PeriodeHistorique
  ): Observable<any> {
    return this.http.get<any>(
      `${this.apiUrl}/historique/${employeeId}?periode=${periode}`
    );
  }

  getHistoriqueParDate(employeeId: number, periode: string): Observable<any> {
    return this.http.get<any>(
      `${this.apiUrl}/historiqueDate/${employeeId}?periode=${periode}`
    );
  }

  replanifierTaches(employeeId: number, strategie: string): Observable<any> {
    return this.http.put<any>(
      `${this.apiUrl}/replanifier/${employeeId}?strategie=${strategie}`,
      {}
    );
  }
  ...
  statsParEmploye(employeeId: number): Observable<any> {
    return this.http.get<any>(
      `${this.apiUrl}/statistiques-globales-employe/${employeeId}`
    );
  }
}
